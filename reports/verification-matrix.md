...
- 採用source / commit: 未commitのためfile hashで識別 (reports/local/m4-01/run.md)。HEAD de87224 docsのみ、実装untracked。commit/pushは別依頼待ち
...
|---|---|---|---|---|---|---|---|---|
| N-MAC | 必須 | arm64 Mac / Xcode26.6 / SDK26.5 / Rust1.96.1 | PASS | PASS | PASS | PASS | PASS | reports/local/nmac-m4-02/run.md UI-01〜06 PASS |
| N-IOS-SIM | 必須 | arm64 Sim / iPhone 16 iOS18.5 (75B3...) / SDK26.5 | PASS | PASS | PASS | PASS | PASS | reports/local/ios-sim-m4-03/run.md PASS (UI-03はself-test内basicで代替) |
| N-ANDROID-EMU | 必須 | arm64-v8a / medium_phone android-36 / NDK28.2 / JDK21 | PASS | PASS | PASS | PASS | PASS | reports/local/emu-m4-04/run.md UI-01〜06 PASS |
...
| W-SAFARI | 必須 | 実Safari 26.6.2 (実測、推測転記なし) | PASS | PASS | PASS | N/A | PASS | reports/local/wsafari-root/subpath/run.md AppleScript全自動 UI-01〜04+W-02 PASS |
| W-IOS | 必須 | Sim Safari (iPhone 16 iOS18.5 booted) / LAN到達 http://<host-lan-ip>:4174/ (例: 192.168.99.239) | PASS | PASS | PASS | N/A | PARTIAL | reports/local/sim-safari-wios: UI-01/UI-02+W-02 PASS (OCR自動)、UI-03〜05はcliclick mapping不安定で要再試行。スクリプトverify-sim-safari.mjsは全自動・証拠付き (`--url` 必須) |
| W-ANDROID | 必須 | Emu Chrome 152.0.7977.82 / medium_phone / 到達 http://10.0.2.2:4173/ | PASS | PASS | PASS | N/A | PASS | reports/local/emu-chrome-wandroid/run.md verify-emu-chrome.mjs全自動 UI-01〜05+W-02 PASS |
...
| N-ANDROID-DEVICE | 後続 | Android実機 (XQ-DQ44 arm64-v8a sdk35 / 実機ロック中、シリアルは記録省略) | BLOCKED | BLOCKED | BLOCKED | BLOCKED | BLOCKED | reports/local/android-device/BLOCKED.md。verify-android-device.mjsで全自動化済み (`--serial`/`--preview-host` 必須、install+launch+dump+ tapsはemuと同一)。再開条件: ロック解除後に npm run verify:android-device -- --serial <adb-serial> --preview-host <host-lan-ip> |
