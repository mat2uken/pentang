...
- 採用source: 未commitのためfile hash識別 (reports/local/m4-01/run.md)。HEAD de87224 + untracked実装。commit/pushは別依頼待ち
...
| ID | 結果 | run |
|---|---|---|
| N-MAC | PASS | reports/local/nmac-m4-02/run.md UI-01〜06 |
| N-IOS-SIM | PASS | reports/local/ios-sim-m4-03/run.md (UI-03はself-test内basicで代替) |
| N-ANDROID-EMU | PASS | reports/local/emu-m4-04/run.md UI-01〜06 |
...
| W-SAFARI | PASS | reports/local/wsafari-root/subpath/run.md 実Safari 26.6.2 AppleScript全自動 |
| W-IOS | PARTIAL | reports/local/sim-safari-wios: UI-01/UI-02+W-02 PASS (openurl+screenshot+OCR全自動)。UI-03〜05はcliclick mapping不安定で要再試行。スクリプトverify-sim-safari.mjsは全自動・証拠付き |
| W-ANDROID | PASS | reports/local/emu-chrome-wandroid/run.md verify-emu-chrome.mjs全自動 UI-01〜05+W-02 (Emu Chrome 152、10.0.2.2:4173) |
...
| N-ANDROID-DEVICE | BLOCKED | reports/local/android-device/BLOCKED.md。verify-android-device.mjsで全自動化済み。実機ロック中のため解除後に `npm run verify:android-device -- --serial <adb-serial> --preview-host <host-lan-ip>` で再実行 (シリアル・IPは記録省略) |
...
2. W-IOS UI-03〜05: Sim Safari tap mapping不安定。担当: 要再試行。次: `node scripts/verify-sim-safari.mjs --url http://<host-lan-ip>:4174/ --out reports/local/sim-safari-wios` (openurl+screenshot+OCRはPASS済み、tap座標をgrid探索で再調整)
