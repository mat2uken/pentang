//! `corebin:` custom schemeによるバイナリデータプレーン。
...
/// corebin schemeハンドラ。同期でよい (計算は数十μs級でblockしない)。
pub fn handle_corebin(req: Request<Vec<u8>>) -> Response<Cow<'static, [u8]>> {
    ...
    match (req.method(), req.uri().path()) {
        ...
        _ => err_json(
            ...
            &AppErrorDto::new("INVALID_ARGUMENT", "unknown corebin route".to_string()),
        ),
    }
}
...
mod tests {
    ...
    fn post_transform(body: Vec<u8>) -> Request<Vec<u8>> {
        Request::builder()
            .method(Method::POST)
            .uri("corebin://localhost/transform")
            .body(body)
            .expect("req")
    }
    ...
    fn transform_round_trip() {
        let res = handle_corebin(post_transform(basic_batch()));
    ...
    fn invalid_body_maps_to_400_json() {
        let res = handle_corebin(post_transform(b"garbage".to_vec()));
    ...
    fn health_and_options_and_unknown_routes() {
        let health = Request::builder()
            .method(Method::GET)
            .uri("corebin://localhost/health")
            .body(Vec::new())
            .expect("req");
        let res = handle_corebin(health);
        assert_eq!(res.status(), StatusCode::OK);
        ...
        let opts = Request::builder()
            ...
            .body(Vec::new())
            .expect("req");
        let res = handle_corebin(opts);
        assert_eq!(res.status(), StatusCode::NO_CONTENT);
        assert!(res.headers().contains_key(ACCESS_CONTROL_ALLOW_ORIGIN));

        let unknown = Request::builder()
            .method(Method::GET)
            .uri("corebin://localhost/nope")
            .body(Vec::new())
            .expect("req");
        assert_eq!(handle_corebin(unknown).status(), StatusCode::NOT_FOUND);
    }
}
