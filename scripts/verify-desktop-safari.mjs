...
function usage() {
  return `usage: node scripts/verify-desktop-safari.mjs [--url http://127.0.0.1:4173/ --out docs/reports/local/wsafari-root]`;
}

const { kv } = parseKVArgs(process.argv.slice(2));
const url = kv["url"] ?? "http://127.0.0.1:4173/";
const outRel = kv["out"] ?? "docs/reports/local/wsafari-root";
