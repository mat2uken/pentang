...
void run_valid(const char* id, const int32_t* in, uint32_t n, int32_t mul,
  ...
  int32_t st = core_transform_i32(n == 0 ? nullptr : in, n, mul, off,
                                 n == 0 ? nullptr : out.data(), n, &sum);
  char buf[128];
  std::snprintf(buf, sizeof(buf), "%s.status", id);
  check_status(st, CORE_OK, buf);
...
int main() {
  // ABI/version
  check(core_abi_version() == 1, "abi", "abiVersion must be 1");
  {
    const char* v = core_version();
  ...
  {
    uint32_t sum = 0xdeadbeef;
    int32_t st = core_transform_i32(nullptr, 0, 1, 2, nullptr, 0, &sum);
    check_status(st, CORE_OK, "core-01.status");
  ...
  // core-02: count=1、input=NULL → INVALID
  {
    ...
    int32_t st = core_transform_i32(nullptr, 1, 1, 0, &out, 1, &sum);
    check_status(st, CORE_ERR_INVALID_ARGUMENT, "core-02.status");
  ...
  // core-03: count=1、output=NULL → INVALID
  {
    int32_t in = 5;
    uint32_t sum = 0x22222222;
    int32_t st = core_transform_i32(&in, 1, 1, 0, nullptr, 1, &sum);
    check_status(st, CORE_ERR_INVALID_ARGUMENT, "core-03.status");
  ...
  // core-04: checksum=NULL → INVALID
  {
    int32_t in = 5;
    int32_t out = 0x11111111;
    int32_t st = core_transform_i32(&in, 1, 1, 0, &out, 1, nullptr);
    check_status(st, CORE_ERR_INVALID_ARGUMENT, "core-04.status");
  ...
  // core-05: cap=0 → BUFFER_TOO_SMALL
  {
    ...
    int32_t out = 0x11111111;
    uint32_t sum = 0x22222222;
    int32_t st = core_transform_i32(&in, 1, 1, 0, &out, 0, &sum);
    check_status(st, CORE_ERR_BUFFER_TOO_SMALL, "core-05.status");
  ...
  {
    uint32_t sum = 0x22222222;
    int32_t st = core_transform_i32(nullptr, 4097, 1, 0, nullptr, 0, &sum);
    check_status(st, CORE_ERR_LIMIT_EXCEEDED, "core-06.status");
  ...
  // core-07: checksum=NULL + cap=0 → INVALIDが先 (capacityより先)
  {
    int32_t in = 5;
    int32_t out = 0x11111111;
    int32_t st = core_transform_i32(&in, 1, 1, 0, &out, 0, nullptr);
    check_status(st, CORE_ERR_INVALID_ARGUMENT, "core-07.status");
    check(out == 0x11111111, "core-07.output-unchanged", "output changed");
  }
  // core-08: input/output=NULL + cap=0 → BUFFER_TOO_SMALLが先
  {
    uint32_t sum = 0x22222222;
    int32_t st = core_transform_i32(nullptr, 1, 1, 0, nullptr, 0, &sum);
    check_status(st, CORE_ERR_BUFFER_TOO_SMALL, "core-08.status");
    check(sum == 0x22222222, "core-08.checksum-unchanged", "checksum changed");
  }
  // core-09: count=0、checksum=NULL → INVALID
  {
    int32_t st = core_transform_i32(nullptr, 0, 1, 0, nullptr, 0, nullptr);
    check_status(st, CORE_ERR_INVALID_ARGUMENT, "core-09.status");
  }
  ...
  // core-10: sentinel不変。成功/失敗の入出力の前後にsentinel。
  {
    ...
    uint32_t sum = 0;
    int32_t st = core_transform_i32(in_vals, 3, 2, 1, &buf[1], 3, &sum);
    check_status(st, CORE_OK, "core-10.ok.status");
    ...
    int32_t in2 = 9;
    st = core_transform_i32(&in2, 1, 1, 0, out2, 0, &sum2);
    check_status(st, CORE_ERR_BUFFER_TOO_SMALL, "core-10.err.status");
  ...
  // maximum-length: 4096件
  {
    ...
    uint32_t sum = 0;
    int32_t st =
        core_transform_i32(in.data(), 4096, 2, 1, out.data(), 4096, &sum);
    check_status(st, CORE_OK, "valid.maximum-length.status");
  ...
  // too-long相当: C側はLIMIT
  {
    uint32_t sum = 0xdeadbeef;
    int32_t st = core_transform_i32(nullptr, 4097, 1, 0, nullptr, 0, &sum);
    check_status(st, CORE_ERR_LIMIT_EXCEEDED, "invalid.too-long.status");
  }
